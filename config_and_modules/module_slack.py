...
def error(error,module_name):
    data = {
        'text':f'*PROJECT: {PROJECT_NAME.upper()}*\n\n*OOPS! an error was found in the module: "{module_name}"*\n\n{error}'
    }
    sendMessage(data)

def nucleiCVE(cve):
    data = {
        "text":f"*QUICK, [{cve}] WAS FOUND ON {PROJECT_NAME.upper()}!!*"
    }
    sendMessage(data)

def nucleiFiles(fileName):
    data = {
        "text":f"*QUICK, [{fileName}] WAS FOUND ON {PROJECT_NAME.upper()}!!*"
    }
    sendMessage(data)

def nucleiVulnerabilities(vuln):
    data = {
        "text":f"*QUICK, [{vuln}] WAS FOUND ON {PROJECT_NAME.upper()}!!*"
    }
    sendMessage(data)

def nucleiSubdomainTakeover():
    data = {
        "text":f"*QUICK, A SUBDOMAIN ON {PROJECT_NAME.upper()} CAN BE TAKEN OVER!!*"
    }
    sendMessage(data)
