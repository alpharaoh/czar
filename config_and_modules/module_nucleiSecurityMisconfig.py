...
def securityMisconfig(target):
    os.system(f"echo '{target}' | nuclei -t {NUCLEI_DIR}/security-misconfiguration/ >> nuclei_SecurityMisconfig.txt") #idgaf

    if os.stat(f"nuclei_SecurityMisconfig.txt").st_size == 0:
        os.system(f"rm nuclei_SecurityMisconfig.txt")
    else:
